...
public interface Student28Repository extends CrudRepository<Student28, Long> {
    ...
    @Query("""
            select s.studentName, (s.monthlySalary * 12) as annualSalary 
            from Student28 s
    ...
    @Query("""
            select new com.demo.project82._28_projections.Student28Pojo (s.studentName, s.monthlySalary * 12) 
            from Student28 s
            where s.studentName=:studentName
            """)
    ...
    @Query("""
            select new com.demo.project82._28_projections.Student28DTO (s.studentName, s.monthlySalary * 12) 
            from Student28 s
            where s.studentName=:studentName
            """)
